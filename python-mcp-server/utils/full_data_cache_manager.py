"""完整数据缓存管理器 - 使用 Redis DB 2。

存储所有缓存数据，提供大容量和历史数据保留。
"""
import logging
import json
...
class FullDataCacheManager(CacheInterface):
    """完整数据缓存管理器 - 使用 Redis DB 2。
    ...
    def __init__(
        self, 
        ttl: int = FULL_DATA_CACHE_TTL, 
        max_size: int = FULL_DATA_CACHE_MAX_SIZE
    ):
        """初始化完整数据缓存。
        ...
        self.namespace = f"full:{APP_ENVIRONMENT}"
        ...
        try:
            ...
            logger.info(f"完整数据缓存已连接到 Redis DB {REDIS_FULL_DATA_DB}")
    ...
    def _evict_lru(self) -> None:
        ...
        try:
            ...
            if key_info:
                ...
                logger.debug(f"LRU 淘汰完整缓存: {oldest_key}")
    ...
    def clear(self) -> None:
        ...
        try:
            ...
            logger.info(f"完整缓存已清空 ({len(keys)} 个键)")
        except redis.exceptions.ConnectionError:
            logger.warning("Redis 连接失败，降级到内存缓存")
            self.use_fallback = True
            self.fallback_cache = CacheManager(ttl=self.ttl, max_size=self.max_size)
    ...
    def get_stats(self) -> Dict:
        ...
        if self.use_fallback:
            ...
            stats['cache_type'] = 'full_data_fallback'
            return stats
        ...
        try:
            ...
            return {
                'cache_type': 'full_data',
                'redis_db': REDIS_FULL_DATA_DB,
